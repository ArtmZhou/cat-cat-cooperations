...
├── cli_agent_output_logs.json   # Agent 输出日志（每 Agent 最多保留 500 条，可配置）
...
每次 Agent 的输出都会被保存为 `StoredCliAgentOutputLog`（每 Agent 默认最多保留 500 条，可通过 `cat.cli.max-output-logs` 配置）：
...
private void saveOutputLog(String agentId, String type, String content) {
    ...
    cleanupOldOutputLogs(agentId); // 保持每 Agent 最多 N 条（默认 500，可配置）
}
```
