...
### 5. 前端界面
- **深色科技风主题**：紫蓝渐变（Violet #7C3AED → Cyan #06B6D4）配色方案
- 定制SVG猫猫图标系统（猫耳、猫爪等科技风元素）
- 毛玻璃（Glassmorphism）效果的顶部导航栏
- 渐变色侧边栏导航，带动态激活指示器
- 深色仪表盘，渐变数值卡片和快速操作入口
- 群聊暗色主题，渐变消息气泡和流式动画
- 响应式布局，支持移动端
...
│   ├── src/
│   │   ├── api/                 # API请求模块
│   │   ├── assets/styles/       # SCSS设计系统（深色科技风）
│   │   ├── components/          # 通用组件（含CatIcons SVG图标）
│   │   ├── views/               # 页面组件
│   │   ├── stores/              # Pinia状态管理
│   │   └── utils/               # 工具函数
│   └── package.json
