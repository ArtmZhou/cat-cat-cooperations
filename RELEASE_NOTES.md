...
- **聊天室会话记录恢复**: 修复聊天室中与Agent聊天后切换到其他页面再切回时会话记录丢失的问题，通过localStorage持久化选中的Agent和消息历史，页面重新进入后自动恢复之前的会话状态
- **聊天室状态隔离**: 修复与agent1对话时agent2页面也显示「发送请求...」的问题，将isLoading和cliStatus从全局状态改为按Agent ID存储的状态Map，各Agent的加载状态和CLI状态完全独立互不影响
- **聊天室错误处理改进**: 修复发送失败时错误信息仅修改内存对象但未持久化到messageHistory的问题，使用updateAssistantMessage统一更新消息和历史
- **聊天室多Agent并行对话**: 修复同时与多个Agent对话时切换Agent导致旧Agent响应丢失的问题，WebSocket订阅不再随Agent切换取消，所有已订阅Agent的输出均被持续接收并保存到messageHistory
- **聊天室执行状态提示**: 新增Agent进入EXECUTING状态时在聊天区显示"执行中..."提示，以及接收响应时显示"接收响应中..."提示，用户全程可感知处理进度
- **聊天室请求耗时显示**: 新增请求耗时计时器，从发送请求开始到响应完成，在状态栏右侧实时显示已用时间（格式：X秒/X分Y秒），支持多Agent独立计时
- **聊天室 / 群聊推送彻底隔离**: 修复群聊中 agent 回复串入聊天室单聊的问题。后端现在会根据群聊上下文路由 WebSocket 推送：群聊执行时不再把 `output / text_delta / error / done / EXECUTING` 推送到聊天室 topic，仅在任务结束后同步最终 `RUNNING` 状态，既保证隔离又不影响聊天室多 Agent 并行对话
